...
export default {
  ...
  methods: {
    checkMode () {
      if (['vertical', 'horizontal'].indexOf(this.mode) === -1 || this.mode === '') {
        this.vMode = 'vertical'
    ...
    checkPlacement () {
      if (this.vMode === 'vertical') {
        if (['top', 'bottom'].indexOf(this.placement) === -1 || this.placement === '') {
          this.placementV = 'top'
          console.error('@ba: placement error in vertical mode, try "top" or "bottom"')
        } else {
          this.placementV = this.placement
        }
      } else {
        if (['left', 'right'].indexOf(this.placement) === -1 || this.placement === '') {
          this.placementV = 'left'
          console.error('@ba: placement error in horizontal mode, try "left" or "right"')
        } else {
          this.placementV = this.placement
        }
      }
    ...
    getFirstBlockStyle () {
      if (this.vMode === 'vertical') {
        return this.placementV === 'top' ? { width: '100%', height: this.spaceV } : { width: '100%', height: this.transformSpace(this.spaceV) }
      } else {
        return this.placementV === 'left' ? { width: this.spaceV, height: '100%', float: 'left' } : { width: this.transformSpace(this.spaceV), height: '100%', float: 'left' }
      }
    },
    getLastBlockStyle () {
      if (this.vMode === 'vertical') {
        return this.placementV === 'top' ? { width: '100%', height: this.transformSpace(this.spaceV) } : { width: '100%', height: this.spaceV }
      } else {
        return this.placementV === 'left' ? { width: this.transformSpace(this.spaceV), height: '100%', float: 'right' } : { width: this.spaceV, height: '100%', float: 'right' }
      }
    },
    getBlockClass () {
      ...
      if (val.indexOf('%') > -1 && old.indexOf('%') > -1) {
        ...
        _steps.forEach((el, index) => {
          if (index < _steps.length - 1) {
            for (let k = 0; k < 20; k++) { // consider 1% as 20px, to achive smoth moving
        ...
        console.error('@ba: need to keep space param unit consistent with "px" or "%"')
      }
    }
  }
}
