<template>
  <div
    id="flowfile-edit"
    v-loading="loading || $store.getters.loadingGetter || xmlLoading"
  ...
  >
    ...
    <b-layout mode="vertical" placement="top" space="54px" :border="false">
      <flowfile-edit-top
        ...
        :current-page-id="drawioTabsPage[currentTabId] || this.$route.query.excel_id"
  ...
  methods: {
    ...
    handleSaveXmlOrImgFromDrawio (params) {
      ...
      if (params.format === 'xml') {
        if (!params.currentPage) { // 表示导出整个 xml
          _params.drawio = params.data
          _params.format = 'xml'
          this.saveFlowfileToServer(id, _params)
        } else { // 表示 导出 当前页的 xml
          // 流程图 -> excel 逻辑
          this.flowchartToExcel({ params })
        }
      }
      if (params.format === 'png') {
        ...
        if (params.hasOwnProperty('action') && params.action === 'get-all-images') {
          // 下载word或pdf时依次导出所有图片并保存到队列中，通过防抖处理等待所有图片都载入队列后执行上传服务器并
          this.imgListForDownload.push({ image: data })
          downloadUtils.saveImgsAsWordOrPdf.call(this)
        } else if (params.hasOwnProperty('action') && params.action === '') {
          // action 为 默认值, 即保存单张图片
          _params.preview = data.substring(data.lastIndexOf(',') + 1)
          this.saveFlowfileToServer(id, _params).then(() => {
            if (params.hasOwnProperty('saveType') && params.saveType === 'normal-save') {
              // 点击保存按钮，或者返回按钮是提示保存的情形
              this.$msgMnger.success('保存成功')
              this.$store.commit('MUTATE_LOADING_STACK', { flag: 'end', module: 'loading-for-save' })
            } else if (params.hasOwnProperty('saveType') && params.saveType === 'auto-save') {
              // 长期未操作，自动保存处理的回调
              this.$refs.flowFileEditTop.autoSaveSuccess()
            }
          })
        }
      }
