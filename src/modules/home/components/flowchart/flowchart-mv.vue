...
export default {
  ...
  methods: {
    ...
    move () {
      if (Number(this.currentDataSpaceGetter.type) !== 2) {
        ...
        if (this.currentDataSpace !== this.currentDataSpaceIdGetter) {
          // 移动到非当前企业空间的文件夹
          this.$msgMnger.error('当前企业数据不能移动到别的企业空间')
          return
        } else {
          this.moveApi()
        }
    ...
    moveApi () {
      ...
      this.$api.moveFile({
        folder_id_new: this.targetId,
        folder_id_old: this.currentHandleFileData.folder_id,
        file_id: this.currentHandleFileData.id,
        is_common: this.currentHandleFileData.is_common
      }).then(res => {
