    ...
    <div id="systemctrl">
        ...
        <el-table
            ...
            <el-table-column
               ...
               >
               <template slot-scope="scope" >
                    <a  class="syctema"  @click="clickSystemName(scope.row)">{{scope.row['matrix_name']}}</a>
                </template>
            </el-table-column>
            <el-table-column
                prop="user_name"
        ...
        <file-upload-comp class="upload" :accept="'.xlsx, .xls, .tar, .rar, .zip'" ref="fileUploadCom" @uploadSuccess="handleUploadSuccess"/>
        <div>
...
export default {
  ...
  methods: {
    ...
    clickSystemName (rows) {
      this.$api.downloadmatrixFile(rows.id).then(res => {
        if (res.code == 0) {
    ...
    getSystemli () {
      ...
      this.$api.getmatrixFiles(params).then(res => {
    ...
    submitHandle () {
      ...
      this.$store.commit('MUTATE_LOADING_STACK', { flag: 'start' })
      $.ajax({
        url: '/api/v2/fc/flow/save/customer_control_matrix_add',
        method: 'post',
        contentType: false,
        processData: false,
        dataType: 'json',
    ...
    handleDelete (data) {
      ...
      this.$confirm('此操作将永久删除该文件, 是否继续?', '提示', {
        ...
        _this.$api.deletematrixFile(data.id).then(res => {
          if (res.code == 0) {
