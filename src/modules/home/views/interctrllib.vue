<template>
    <div id="systemctrl">
        <div class="addnewrisk">
            ...
            <el-button type="primary" class="w90btn" style="height:40px" @click="addNewhandbook">新建手册</el-button>
        </div>
        <div  style="text-align: left;">
            <el-select  class="flodselect"  v-model="manual_level" filterable  clearable @change="manualLevelChange"  placeholder="请选择手册类型">
                <el-option v-for="it in manulLevelList" :key="it.id" :label="it.name" :value="it.id"></el-option>
            </el-select>
            <el-select  class="flodselect"  v-model="manual_type" filterable  clearable @change="manualTypeChange"  placeholder="请选择手册级别">
                <el-option v-for="it in manualTypeList" :key="it.id" :label="it.name" :value="it.id"></el-option>
            </el-select>
            <el-input  class="elinput" clearable v-model="manual_name" placeholder="请输入文件名称"></el-input>
        ...
        <el-table
            ...
            <el-table-column
               ...
               >
                <template slot-scope="scope" >
                    <a  class="syctema"  @click="clickSystemName(scope.row)">{{scope.row['manual_name']}}</a>
                </template>
            </el-table-column>
            <el-table-column
                prop="user_name"
            ...
            <el-table-column
               ...
               >
                <template slot-scope="scope" >
                    {{scope.row['manual_level']?manulLevelList.find(v=>v.id == scope.row['manual_level'])['name']:''}}
                </template>
            </el-table-column>
            <el-table-column
                prop="upload_date"
            ...
            <el-table-column   width="100" prop="action" label="操作">
                <template slot-scope="scope" >
                    <span class="setspan">
                        <i  class="el-icon-edit iconscope" title="编辑"  @click="editHandbook(scope.row)"></i>
                        <i  class="el-icon-download iconscope mr10" title="下载"  @click="suredownurl(scope.row)"></i>
                        <i  class="el-icon-delete iconscope" title="删除"  @click="formatDelete(scope.row)"></i>
                    </span>
                </template>
            </el-table-column>
        ...
        <div></div>
        <el-select  class="flodselect mb10"  v-model="manual_level" filterable  clearable  placeholder="请选择手册级别">
            <el-option v-for="it in manulLevelList" :key="it.id" :label="it.name" :value="it.id"></el-option>
        </el-select>
        <file-upload-comp class="upload"  ref="fileUploadCom" @uploadSuccess="handleUploadSuccess"/>
        <div>
        ...
        <el-dialog
           ...
           <div style="margin-top: 16px;">
            <el-radio-group v-model="radio">
              <el-radio label="handbook">内控手册</el-radio>
              <el-radio label="judge-handbook">评价手册</el-radio>
              <el-radio label="auth-handbook">权限手册</el-radio>
            </el-radio-group>
           </div>
           <div style="text-align: center" slot="footer">
                <el-button @click="createBookVisible = false">取 消</el-button>
                <el-button type="primary" @click="selectHandbook">确 定</el-button>
            </div>
        </el-dialog>
    </div>
</template>
...
const handbookObj = {
  1: {
    editCallback: 'commonEditHandbook',
    type: 'handbook',
    customEdit: 'customEditHandbook',
    customDelete: 'customDeleteHandbook'
  },
  2: {
    editCallback: 'commonEditHandbook',
    type: 'judge-handbook'
...
export default {
  ...
  data () {
    return {
      ...
      manual_level: '', // 手册级别
      manual_type: '', // 手册类型
      manulLevelList: [{ name: '公司层级', id: 1 }, { name: '部门层级', id: 2 }, { name: '岗位层级', id: 3 }], // 手册列表
      manualTypeList: [{ name: '内控手册', id: 1 }, { name: '评价手册', id: 2 }, { name: '权限手册', id: 3 }], // 手册类型列表
      owner_id: '', // 创建者id
      timeRange: [], // 选择时间
      ownerList: [], // 创建者列表,
      createBookVisible: false, // 创建手册类型选择弹窗
      radio: 'handbook', // 手册类型
      editLock: false
    }
  },
  ...
  methods: {
    getManualIdByOtherId (id, cb) {
      this.$api.customEditHandbook({
        manual_id: id
      }).then(res => {
        const { code, data } = res
        if (code === 0) {
        ...
        this.$msgMnger.error(err.msg || '获取手册id失败')
      })
    },
    customEditHandbook (rows) {
      const { type } = handbookObj[rows.type]
      this.getManualIdByOtherId(rows.id, (id) => {
        this.commonEditHandbook({
          name: type,
          query: {
            manual_id: id
          }
    ...
    commonEditHandbook (params) {
      this.editLock = false
      this.$router.push(params)
    },
    editHandbook (rows) {
      if (this.editLock === true) {
        return this.$msgMnger.warn('正在跳转中，请耐心等待。')
      }
      if (rows.manual_type === 2) {
        this.editLock = true
        const currentHandbook = handbookObj[rows.type]
        if (currentHandbook && this[currentHandbook.customEdit]) {
          this[currentHandbook.customEdit](rows)
          return
        }
        this[currentHandbook.editCallback]({
          name: currentHandbook.type,
          query: {
            manual_id: rows.id || ''
          }
        })
      } else {
        this.$msgMnger.warn('该手册不可编辑')
      }
    },
    // 点击控制制度
    clickSystemName (rows) {
      this.$api.downloadInterNallibs(rows.id).then(res => {
        if (res.code == 0) {
      ...
      // }
      // this.$api.getctrlManualName(params).then(res=>{
      //     if(res.code == 0){
      //         window.open(window.location.origin+'/onlinePreview?url='+encodeURIComponent(Base64.encodeURI(`${window.location.origin}/fcFlowfilePdf/生产部内控手册2022版.rar/954?type=3`)))
      //         // window.open(`/fcFlowfilePdf/${coninfo}/${this.currentDataSpaceIdGetter}?type=3`)
    ...
    // 选择手册级别变化
    manualLevelChange (val) {
      this.page = 1
      this.getSystemli()
    },
    manualTypeChange (val) {
      this.page = 1
      this.getSystemli()
    },
    // 选择时间变化
    ...
    getSystemli () {
      let params = {
        ...
        manual_level: this.manual_level || '',
        type: this.manual_type || '',
        secondary_id: this.secondary_id || ''
      }
      this.$store.commit('MUTATE_LOADING_STACK', { flag: 'start' })
      this.$api.getInterNallibs(params).then(res => {
    ...
    submitHandle () {
      ...
      if (!this.manual_level) {
        this.$message.error('请先选择手册级别！')
        return
      }
      ...
      _formData.append('manual_level', this.manual_level)
      fileArr.forEach(el => {
      ...
      this.$store.commit('MUTATE_LOADING_STACK', { flag: 'start' })
      $.ajax({
        url: '/api/v2/fc/flow/save/internal_control_manual_add',
        method: 'post',
        contentType: false,
        processData: false,
        dataType: 'json',
    ...
    customDeleteHandbook (rows) {
      this.handleDelete({ id: rows.id })
    },
    formatDelete (rows) {
      const currentHandbook = handbookObj[rows.type]
      if (currentHandbook && this[currentHandbook.customEdit]) {
        this[currentHandbook.customDelete](rows)
        return
      }
    ...
    handleDelete (data) {
      ...
      this.$confirm('此操作将永久删除该文件, 是否继续?', '提示', {
        ...
        this.getManualIdByOtherId(data.id, (id) => {
          this.$api.deleteManualHandbook({ id })
        })
        _this.$api.deleteInterNallibs(data.id).then(res => {
          if (res.code == 0) {
    ...
    selectHandbook () {
      ...
      this.$router.push({ name: this.radio })
    }
  }
}
</script>
