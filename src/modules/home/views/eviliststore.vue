<template>
    <div id="systemctrl">
        <div class="addnewrisk">
            <div class="serach">
                <el-input  class="elinput" clearable v-model="riskName" placeholder="请输入流程名称"></el-input>
        ...
        <el-table
            ...
            <el-table-column
                ...
                >
                <template slot-scope="scope" >
                    {{scope.row.control_evidence}}
                    <!-- {{list.filter(v=>v.excel_id==scope.row['control_parent'])[0]['control_evidence']}} -->
                </template>
            </el-table-column>
...
export default {
  ...
  methods: {
    ...
    suredownurl () {
      ...
      this.$api.downeviInfoli(params).then(res => {
        if (res.code == 0) {
          this.isDownTimes = false
          let downloadURL = res.data + `?_token=${this.token}`
          this.$api.accountdownlibrary({ company_id: this.currentDataSpaceIdGetter, flow_name: '控制证据库', lines: 1 }).then()
    ...
    getSystemli () {
      ...
      this.$api.geteviInfoli(params).then(res => {
