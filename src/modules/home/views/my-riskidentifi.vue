...
export default {
  ...
  methods: {
    ...
    clickSystemName (rows) {
      this.$api.downriskidentifi(rows.id).then(res => {
        if (res.code == 0) {
    ...
    getSystemli () {
      ...
      this.$api.showriskidentifili(params).then(res => {
    ...
    submitHandle () {
      ...
      this.$store.commit('MUTATE_LOADING_STACK', { flag: 'start' })
      $.ajax({
        url: '/api/v2/fc/flow/add_risklib',
        method: 'post',
        contentType: false,
        processData: false,
        dataType: 'json',
    ...
    handleDelete (data) {
      ...
      this.$confirm('此操作将永久删除该文件, 是否继续?', '提示', {
        ...
        _this.$api.deleteriskidentifi(data.id).then(res => {
          if (res.code == 0) {
