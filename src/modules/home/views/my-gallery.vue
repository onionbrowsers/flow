    ...
    <div id="systemctrl">
        <div class="addnewrisk">
             <div class="serach">
                ...
                <el-select  class="flodselect" filterable  clearable @change="flodChangeId" v-model="flodId" placeholder="请选择所属部门">
        ...
        <div style="text-align: left;">
            <el-input  class="elinput" clearable v-model="visioName" placeholder="请输入文件名称"></el-input>
        ...
        <el-table
            ...
            <el-table-column
               ...
               >
               <template slot-scope="scope" >
                    <a  class="syctema"  @click="clickSystemName(scope.row)">{{scope.row['visio_name']}}</a>
                </template>
            </el-table-column>
            <el-table-column
                prop="user_name"
        ...
        <file-upload-comp class="upload" :accept="'.vsd, .vsdx, .tar, .rar, .zip'" ref="fileUploadCom" @uploadSuccess="handleUploadSuccess"/>
        <div>
...
export default {
  ...
  methods: {
    ...
    clickSystemName (rows) {
      this.$api.downloadImgLibrary(rows.id).then(res => {
        if (res.code == 0) {
    ...
    getSystemli () {
      ...
      this.$api.getImgLibrary(params).then(res => {
    ...
    submitHandle () {
      ...
      this.$store.commit('MUTATE_LOADING_STACK', { flag: 'start' })
      $.ajax({
        url: '/api/v2/fc/flow/create_visio_library',
        method: 'post',
        contentType: false,
        processData: false,
        dataType: 'json',
    ...
    handleDelete (data) {
      ...
      this.$confirm('此操作将永久删除该文件, 是否继续?', '提示', {
        ...
        _this.$api.deleteImgLibrary(data.id).then(res => {
          if (res.code == 0) {
