    ...
    <div class="folder-table-wrapper">
        <div class="filter-folder-table">
            <div class="search-wrapper">
                <span>目录位置：</span>
                <el-select style="margin-right: 16px;" v-model="selectValue" placeholder="请选择目录位置">
                    <el-option
        ...
        >
            ...
            <el-table-column align="center" prop="catalog_name" label="目录位置"></el-table-column>
            <el-table-column align="center" prop="source" label="所属部门"></el-table-column>
...
export default {
  ...
  methods: {
    ...
    getTableData () {
      this.$api.getJudgeHandbookTableData({
        company_id: this.currentDataSpaceIdGetter,
        page: this.current,
        page_size: this.size,
        catalog_id: this.selectValue,
        catalog_name: this.searchText,
    ...
    getSelectOptions () {
      this.$api.getJudgeHandbookCatalog({
        manual_id: this.manualId,
        company_id: this.currentDataSpaceIdGetter
      }).then(res => {
        const { code, data } = res
  ...
  watch: {
    currentDataSpaceIdGetter (oldValue, newValue) {
      ...
      if (oldValue !== newValue && this.$route.query.manual_id) {
