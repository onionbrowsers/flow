<template>
    <div id="systemctrl">
        <div class="addnewrisk">
             <div class="serach">
                <el-date-picker
                 ...
                 <el-select  class="flodselect" style="margin-bottom:10px" clearable filterable @change="flodChangeId" v-model="flodId" placeholder="请选择所属部门">
                    <el-option v-for="it in flodList" :key="it.id" :label="it.name" :value="it.id"></el-option>
                </el-select>
                <el-select  class="flodselect"  filterable  clearable @change="ownerChange" v-model="owner_id" placeholder="请选择创建者">
        ...
        <div style="text-align: left;">
            <el-input  class="elinput" clearable v-model="sysName" placeholder="请输入制度名称"></el-input>
        ...
        <el-table
            ...
            <el-table-column
               ...
               >
                <template slot-scope="scope" >
                    <a  class="syctema"  @click="clickSystemName(scope.row)">{{scope.row['system_name']}}</a>
                </template>
            </el-table-column>
            <!-- <el-table-column
                prop="localpath"
...
export default {
  ...
  methods: {
    ...
    suredownurl (rows) {
      ...
      // }
      // this.$api.downMySystemInfo(params).then(res=>{
      this.$api.downByidSystemInfo(rows.id).then(res => {
        if (res.code == 0) {
    ...
    getSystemli () {
      ...
      this.$api.getMySystemInfo(params).then(res => {
    ...
    submitHandle () {
      ...
      this.$store.commit('MUTATE_LOADING_STACK', { flag: 'start' })
      $.ajax({
        url: '/api/v2/fc/flow/save/flow_system_add',
        method: 'post',
        contentType: false,
        processData: false,
        dataType: 'json',
